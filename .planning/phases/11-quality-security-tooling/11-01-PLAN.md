...
must_haves:
  truths:
    ...
    - "CI build job runs spotlessCheck so unformatted code cannot bypass the pre-commit hook via --no-verify"
  artifacts:
    ...
    - path: ".github/workflows/ci.yml"
      provides: "CI spotlessCheck step in build job"
  ...
  key_links:
    ...
    - from: "build.gradle.kts"
      ...
      via: "Manual reference: txt entries are reflected as disable() calls in build.gradle.kts"
      pattern: "disable\\("
---
...
<task type="auto">
  ...
  <action>
    ...
    1. **gradle/libs.versions.toml** — Add version entries:
       ...
       Add library entry for errorprone-core only (nullaway library entry belongs to Plan 02). Add plugin entries for errorprone and spotless.
...
<task type="auto">
  ...
  <action>
    ...
    6. **Enforce spotlessCheck in CI** — Add a Spotless verification step to `.github/workflows/ci.yml` in the `build` job, immediately after the "Compile all sources" step:
       ```yaml
       - name: Check code formatting
         run: ./gradlew spotlessCheck --console=plain
       ```
       Per locked decision: "Build behavior identical local/CI — ./gradlew build fails on violations everywhere, no warning-only mode." This ensures developers who bypass the pre-commit hook with `--no-verify` will still have unformatted code caught by CI. The step runs after compilation so that spotlessCheck can verify all source files.

    7. **Verify clean build** — Run `./gradlew build -x integrationTest` to confirm everything passes with Error Prone + Spotless active.
  </action>
  <verify>
