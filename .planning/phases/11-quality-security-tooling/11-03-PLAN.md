...
must_haves:
  truths:
    ...
    - "OWASP Dependency-Check also runs in the CI security job via ./gradlew dependencyCheckAnalyze"
...
<task type="auto">
  ...
  <action>
    ...
    2. **.github/workflows/ci.yml** — Add a new `security` job after the `build` job:
       ...
       - OWASP runs first via `./gradlew dependencyCheckAnalyze` (per locked decision: OWASP runs locally AND in CI)
       - The Trivy filesystem scan catches Java dependency vulnerabilities
       - Three Trivy image scans: pgvector/pgvector:pg16, unclecode/crawl4ai:0.8.0, and the built app image
       - `exit-code: 1` means the Trivy job fails on HIGH/CRITICAL findings
  ...
  <verify>
    ...
    Verify `.github/workflows/ci.yml` contains the security job with OWASP step + 4 Trivy scan steps (filesystem + 3 images).
    Verify the security job includes `setup-java`, `setup-gradle`, artifact restore, and `./gradlew dependencyCheckAnalyze`.
  ...
  <done>
    Security CI job runs OWASP Dependency-Check and Trivy. Trivy scans Java filesystem and all 3 Docker images (pgvector, crawl4ai, app) for HIGH/CRITICAL CVEs. OWASP runs via ./gradlew dependencyCheckAnalyze. .trivyignore file exists for suppressions. CI pipeline fails on security findings.
  </done>
</task>
...
3. CI YAML contains security job with OWASP step + Trivy scanning filesystem + 3 images (SECU-01, SECU-02)
