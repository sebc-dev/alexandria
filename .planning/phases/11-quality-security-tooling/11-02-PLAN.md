...
<task type="auto">
  ...
  <action>
    ...
    2. **Annotate all nullable parameters, return types, and fields** across ALL Java files:
       ...
       Focus areas (42 main Java files + ~24 test files + ~10 integration test files):
       ...
       - **Test classes** (src/test/java/**/*.java): NullAway analyzes test code under dev.alexandria.* because AnnotatedPackages=dev.alexandria covers all source sets. Add @Nullable annotations to test helper methods, mock return values, and test fixtures where null is intentional.
       - **Integration test classes** (src/integrationTest/java/**/*.java): Same NullAway analysis applies. Annotate nullable test parameters and setup methods.
