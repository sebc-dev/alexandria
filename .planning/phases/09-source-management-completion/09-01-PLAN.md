...
must_haves:
  truths:
    - "document_chunks rows get source_id populated during ingestion via post-insert batch UPDATE"
    - "CrawlProgressTracker supports crawl cancellation via cancelCrawl/isCancelled methods"
    - "Flyway V4 migration deletes orphan chunks (source_id IS NULL) on startup"
    - "sourceId flows from CrawlService through IngestionService to storeChunks"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    *IngestionServiceTest* (update existing):
    - Update constructor in test setup to pass mock DocumentChunkRepository
