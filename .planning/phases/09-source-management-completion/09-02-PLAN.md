...
Purpose: Closes SRC-02 (accurate list_sources), SRC-03 (cascade delete with feedback), SRC-04 (last_crawled_at exposure), and SRC-05 (index statistics). SRC-01 (add_source) is already functional from Phase 7, verified by 7 existing McpToolServiceTest tests -- no new code needed, coverage is explicit.
...
<task type="auto">
  ...
  <action>
    ...
    **3. crawlStatus() / formatCompletedSummary()** -- Replace `source.getChunkCount()` with real count and breakdown format:
    - Call `formatChunkCount(source.getId())` instead of `source.getChunkCount()`
    - Update the format string label from `"Chunks: %d"` to `"Chunks: %s"` to accept the breakdown string (e.g., `"1247 (892 prose, 355 code)"`)
    - The completed summary format becomes: `"Source: %s (%s)%nStatus: %s%nChunks: %s%nLast crawled: %s"` with `formatChunkCount(source.getId())` as the chunks argument
...
<task type="auto">
  ...
  <action>
    ...
    *crawlStatus:*
    - Update `crawlStatusShowsCompletedSummary` (if it exists) to verify real chunk count with breakdown format from formatChunkCount instead of source.getChunkCount(). Mock `countBySourceIdGroupedByContentType` to return `[["prose", 50], ["code", 20]]`. Assert output contains `"Chunks: 70 (50 prose, 20 code)"` (the breakdown string, not a bare integer).
...
<success_criteria>
- SRC-01: add_source functional (already working from Phase 7; verified by existing McpToolServiceTest tests: addSourceCreatesSourceWithScopeAndTriggersCrawl, addSourceWithNullScopeUsesDefaults, addSourceReturnsImmediatelyWithId, addSourceWithBlankUrlReturnsError, addSourceWithLlmsTxtUrlSavesIt, addSourceSetsVersionOnEntity, addSourceWithNullVersionLeavesVersionNull)
