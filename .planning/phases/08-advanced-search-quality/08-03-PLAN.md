...
must_haves:
  truths:
    - "SearchRequest accepts optional filter parameters: source, sectionPath, version, contentType"
  ...
  artifacts:
    - path: "src/main/java/dev/alexandria/search/SearchRequest.java"
      ...
      contains: "source.*sectionPath.*version.*contentType"
  ...
  <action>
    **TDD RED -- write failing tests:**
    ...
    1. Update `SearchRequestTest.java` with new tests:
       - `filterFieldsDefaultToNull()`: verify that SearchRequest(query, 10) has null source, sectionPath, version, contentType, minScore
       - `builderSetsAllFilterFields()`: verify a builder/constructor sets all fields correctly
...
- SearchRequest has source, sectionPath, version, contentType, minScore fields
