...
<task type="auto">
  ...
  <action>
    ...
    Note: The batch update queries use source_url metadata (not source_id) because document_chunks has no source_id column -- chunks are linked to sources via source_url in JSONB metadata.
  </action>
  <verify>
