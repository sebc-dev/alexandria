# Requirements Archive: v1.5 Full RAG System
...
- [x] **SRC-01**: User can add a documentation URL as a source via MCP tool `add_source`
- [x] **SRC-02**: User can list all configured sources with status, last crawl time, and chunk count via MCP tool `list_sources`
- [x] **SRC-03**: User can remove a source and all its indexed data (cascade delete) via MCP tool `remove_source`
- [x] **SRC-04**: User can see freshness status of each source (time since last crawl, staleness indicator)
- [x] **SRC-05**: User can view index statistics (total chunks, total sources, storage size, embedding dimensions) via MCP tool
...
- [x] **MCP-05**: Server exposes maximum 7 tools: `search_docs`, `list_sources`, `add_source`, `remove_source`, `crawl_status`, `recrawl_source`, `index_statistics` *(expanded from 6 to satisfy SRC-05)*
...
*Last updated: 2026-02-20 after v1.5 milestone completion (SRC-01..05 satisfied, MCP-05 expanded to 7 tools)*
