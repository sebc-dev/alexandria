...
class HybridSearchIT extends BaseIntegrationTest {
    ...
    static final String CHUNK_A_TEXT = "To configure routing in Spring Boot, use @RequestMapping annotation "
    ...
    static final String CHUNK_B_TEXT = "The RouterModule in Angular provides directives and services for "
    ...
    static final String CHUNK_C_TEXT = "PostgreSQL supports JSONB columns for storing semi-structured data. "
    ...
    void semantic_search_finds_relevant_chunks_by_meaning() {
        ...
        boolean hasRoutingChunk = results.stream()
                .limit(3)
                .anyMatch(r -> r.text().contains("routing") || r.text().contains("Routing")
                        || r.text().contains("RouterModule") || r.text().contains("RequestMapping"));
    ...
    void keyword_search_finds_chunks_with_exact_terms() {
        ...
        boolean containsChunkB = results.stream()
                .anyMatch(r -> r.text().contains("RouterModule"));
        assertThat(containsChunkB).as("Results should include chunk B with exact term 'RouterModule'").isTrue();

        // Chunk B should have a non-zero score
        SearchResult chunkBResult = results.stream()
                .filter(r -> r.text().contains("RouterModule"))
                .findFirst()
                .orElseThrow();
    ...
    void hybrid_search_combines_vector_and_keyword_results() {
        ...
        // Chunk B (Angular RouterModule) should rank highly because it matches BOTH semantically AND by keyword
        boolean chunkBInTop3 = results.stream()
                .limit(3)
                .anyMatch(r -> r.text().contains("RouterModule"));
        assertThat(chunkBInTop3).as("Chunk B should rank in top 3 due to both semantic and keyword match").isTrue();
    }
    ...
    void search_respects_max_results_parameter() {
        ...
        List<SearchResult> defaultResults = searchService.search(
                new SearchRequest("programming documentation guide")
        );
