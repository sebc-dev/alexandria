...
class IngestionServiceIT extends BaseIntegrationTest {
  ...
  void ingestPageProducesSearchableChunks() {
    ...
    List<SearchResult> results =
        searchService.search(new SearchRequest("Spring auto-configuration"));
    assertThat(results).isNotEmpty();
    assertThat(results).anyMatch(r -> "https://docs.spring.io/config".equals(r.sourceUrl()));
  }
  ...
  void ingestMultiplePages() {
    ...
    int chunkCount1 =
        ingestionService.ingestPage(markdown1, "https://docs.example.com/database", timestamp);
    int chunkCount2 =
        ingestionService.ingestPage(markdown2, "https://docs.example.com/auth", timestamp);
    assertThat(chunkCount1 + chunkCount2).isGreaterThanOrEqualTo(2); // at least 1 chunk per page

    List<SearchResult> dbResults =
        searchService.search(new SearchRequest("PostgreSQL connection pooling"));
    assertThat(dbResults).anyMatch(r -> "https://docs.example.com/database".equals(r.sourceUrl()));

    List<SearchResult> authResults = searchService.search(new SearchRequest("JWT authentication"));
    assertThat(authResults).anyMatch(r -> "https://docs.example.com/auth".equals(r.sourceUrl()));
  }
