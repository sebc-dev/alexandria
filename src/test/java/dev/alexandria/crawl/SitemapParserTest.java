...
class SitemapParserTest {
    ...
    void setUp() {
        ...
        lenient().when(requestHeadersUriSpec.uri(anyString())).thenAnswer(invocation -> {
            String url = invocation.getArgument(0);
            Object registered = urlResponses.get(url);

            @SuppressWarnings("unchecked")
            RestClient.RequestHeadersSpec<? extends RestClient.RequestHeadersSpec<?>> headersSpec =
    ...
    void discoverFromSitemapValidSitemapReturnsUrls() {
        String sitemapXml = """
                ...
                """;
        stubHttpGet("https://docs.example.com/sitemap.xml", sitemapXml.getBytes(StandardCharsets.UTF_8));
    ...
    void discoverFromSitemapSitemapIndexReturnsUrlsFromNestedSitemaps() {
        ...
        String nestedSitemap = """
                ...
                """;
        stubHttpGet("https://docs.example.com/sitemap.xml", sitemapIndex.getBytes(StandardCharsets.UTF_8));
        stubHttpGet("https://docs.example.com/sitemap-docs.xml", nestedSitemap.getBytes(StandardCharsets.UTF_8));
    ...
    void discoverFromSitemapFirstCandidateFailsTriesSecond() {
        String sitemapXml = """
                ...
                """;
        stubHttpGetException("https://docs.example.com/sitemap.xml");
        stubHttpGet("https://docs.example.com/sitemap_index.xml", sitemapXml.getBytes(StandardCharsets.UTF_8));
