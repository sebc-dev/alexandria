...
class CrawlServiceTest {
  ...
  @Mock private Crawl4AiClient crawl4AiClient;

  @Mock private PageDiscoveryService pageDiscoveryService;

  @Mock private CrawlProgressTracker progressTracker;

  @Mock private IngestionService ingestionService;

  @Mock private IngestionStateRepository ingestionStateRepository;
  ...
  void crawlSiteHappyPathReturnsSinglePageResult() {
    ...
    var discovery =
        new PageDiscoveryService.DiscoveryResult(
    ...
    when(crawl4AiClient.crawl("https://docs.example.com/guide"))
        .thenReturn(
            new CrawlResult("https://docs.example.com/guide", "# Guide", List.of(), true, null));
  ...
  void crawlSiteLinkCrawlModeFollowsInternalLinks() {
    ...
    var discovery =
        new PageDiscoveryService.DiscoveryResult(
            List.of(), PageDiscoveryService.DiscoveryMethod.LINK_CRAWL, null);
    when(pageDiscoveryService.discoverUrls(rootUrl)).thenReturn(discovery);
    ...
    when(crawl4AiClient.crawl("https://docs.example.com/page1"))
        .thenReturn(
            new CrawlResult(
                ...
                "# Page 1",
                List.of("https://docs.example.com/page2"),
                true,
                null));
  ...
  void crawlSiteFailedCrawlResultSkippedAndContinues() {
    ...
    var discovery =
        new PageDiscoveryService.DiscoveryResult(
            List.of("https://docs.example.com/fail", "https://docs.example.com/ok"),
            PageDiscoveryService.DiscoveryMethod.SITEMAP,
            null);
    when(pageDiscoveryService.discoverUrls(rootUrl)).thenReturn(discovery);
    when(crawl4AiClient.crawl("https://docs.example.com/fail"))
        .thenReturn(
            new CrawlResult("https://docs.example.com/fail", null, List.of(), false, "404"));
  ...
  void crawlSiteExceptionDuringCrawlSkippedAndContinues() {
    ...
    var discovery =
        new PageDiscoveryService.DiscoveryResult(
            List.of("https://docs.example.com/error", "https://docs.example.com/ok"),
            PageDiscoveryService.DiscoveryMethod.SITEMAP,
            null);
    when(pageDiscoveryService.discoverUrls(rootUrl)).thenReturn(discovery);
  ...
  void scopeFilterRejectsBlockedUrls() {
    ...
    var discovery =
        new PageDiscoveryService.DiscoveryResult(
            List.of("https://docs.example.com/guide", "https://docs.example.com/api/internal"),
            PageDiscoveryService.DiscoveryMethod.SITEMAP,
            null);
    when(pageDiscoveryService.discoverUrls(rootUrl)).thenReturn(discovery);
    when(crawl4AiClient.crawl("https://docs.example.com/guide"))
    ...
    when(ingestionService.ingestPage(
            any(UUID.class), anyString(), anyString(), anyString(), isNull(), isNull()))
        .thenReturn(1);
  ...
  void scopeFilterAllowsMatchingUrls() {
    ...
    var discovery =
        new PageDiscoveryService.DiscoveryResult(
            List.of("https://docs.example.com/docs/guide", "https://docs.example.com/blog/post"),
            PageDiscoveryService.DiscoveryMethod.SITEMAP,
            null);
    when(pageDiscoveryService.discoverUrls(rootUrl)).thenReturn(discovery);
    when(crawl4AiClient.crawl("https://docs.example.com/docs/guide"))
        .thenReturn(
            new CrawlResult(
                "https://docs.example.com/docs/guide", "# Guide", List.of(), true, null));
    when(ingestionStateRepository.findBySourceIdAndPageUrl(any(), anyString()))
        .thenReturn(Optional.empty());
    when(ingestionService.ingestPage(
            any(UUID.class), anyString(), anyString(), anyString(), isNull(), isNull()))
    ...
    when(crawl4AiClient.crawl("https://docs.example.com/"))
        .thenReturn(
            new CrawlResult(
                "https://docs.example.com/",
                "# Home",
                List.of("https://docs.example.com/level1"),
                true,
                null));
    when(crawl4AiClient.crawl("https://docs.example.com/level1"))
        .thenReturn(
            new CrawlResult(
                "https://docs.example.com/level1",
                "# Level 1",
                List.of("https://docs.example.com/level2"),
                true,
                null));
    when(ingestionStateRepository.findBySourceIdAndPageUrl(any(), anyString()))
        .thenReturn(Optional.empty());
  ...
  void progressTrackerUpdatedDuringCrawl() {
    ...
    when(crawl4AiClient.crawl("https://docs.example.com/page1"))
        .thenReturn(
            new CrawlResult("https://docs.example.com/page1", "# Page", List.of(), true, null));
    when(ingestionStateRepository.findBySourceIdAndPageUrl(any(), anyString()))
        .thenReturn(Optional.empty());
    when(ingestionService.ingestPage(
            any(UUID.class), anyString(), anyString(), anyString(), isNull(), isNull()))
  ...
  void llmsFullContentIngestedDirectlyAndCoveredUrlsSkipped() {
    ...
    var discovery =
        new PageDiscoveryService.DiscoveryResult(
            ...
            PageDiscoveryService.DiscoveryMethod.LLMS_FULL_TXT,
            "# Full content\nAll docs here");
    when(pageDiscoveryService.discoverUrls(rootUrl)).thenReturn(discovery);
    when(ingestionService.ingestPage(
            any(UUID.class), anyString(), anyString(), anyString(), isNull(), isNull()))
  ...
  void cleanupDeletedPagesRemovesOrphanedChunksAndState() {
    ...
    when(crawl4AiClient.crawl("https://docs.example.com/current"))
        .thenReturn(
            new CrawlResult(
                "https://docs.example.com/current", "# Current", List.of(), true, null));
    when(ingestionStateRepository.findBySourceIdAndPageUrl(any(), anyString()))
        .thenReturn(Optional.empty());
    when(ingestionService.ingestPage(
            any(UUID.class), anyString(), anyString(), anyString(), isNull(), isNull()))
    ...
    var currentState =
        new IngestionState(sourceId, "https://docs.example.com/current", "currenthash");
  ...
  void incrementalIngestionSkipsUnchangedContent() {
    ...
    when(crawl4AiClient.crawl("https://docs.example.com/guide"))
        .thenReturn(
            new CrawlResult("https://docs.example.com/guide", markdown, List.of(), true, null));
    // Existing state with same hash
    var existingState = new IngestionState(sourceId, "https://docs.example.com/guide", hash);
    when(ingestionStateRepository.findBySourceIdAndPageUrl(
            sourceId, "https://docs.example.com/guide"))
    ...
    verify(ingestionService, never())
        .ingestPage(any(), anyString(), anyString(), anyString(), any(), any());
  ...
  void incrementalIngestionReprocessesChangedContent() {
    ...
    when(crawl4AiClient.crawl("https://docs.example.com/guide"))
        .thenReturn(
            new CrawlResult(
                "https://docs.example.com/guide", "# Updated Guide", List.of(), true, null));
    // Existing state with different hash
    var existingState =
        new IngestionState(
            sourceId, "https://docs.example.com/guide", ContentHasher.sha256("# Old Guide"));
    when(ingestionStateRepository.findBySourceIdAndPageUrl(
            sourceId, "https://docs.example.com/guide"))
        .thenReturn(Optional.of(existingState));
    when(ingestionService.ingestPage(
    ...
    verify(ingestionService)
        .ingestPage(
            eq(sourceId),
            eq("# Updated Guide"),
            eq("https://docs.example.com/guide"),
            anyString(),
            isNull(),
            isNull());
