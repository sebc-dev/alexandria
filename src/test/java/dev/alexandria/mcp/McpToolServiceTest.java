...
class McpToolServiceTest {
  ...
  @Mock TokenBudgetTruncator truncator;

  @Mock CrawlService crawlService;

  @Mock CrawlProgressTracker progressTracker;
  ...
  @Captor ArgumentCaptor<dev.alexandria.search.SearchRequest> searchRequestCaptor;
  ...
  /**
   * Suppress async dispatch for tests that call addSource or recrawlSource. Must be called before
   * invoking the method under test.
   */
  private void suppressAsyncDispatch() {
    doNothing()
        .when(mcpToolService)
        .dispatchCrawl(any(), anyString(), any(CrawlScope.class), anyBoolean());
  }
  ...
  void addSourceCreatesSourceWithScopeAndTriggersCrawl() {
    ...
    assertThat(output).contains("Spring Docs");
    verify(mcpToolService)
        .dispatchCrawl(any(), eq("https://docs.spring.io"), any(CrawlScope.class), eq(false));
  }
  ...
  void addSourceWithNullScopeUsesDefaults() {
    ...
    String output =
        mcpToolService.addSource(
            "https://docs.spring.io", "Spring Docs", null, null, null, null, null, null);
  ...
  void addSourceWithBlankUrlReturnsError() {
    String output = mcpToolService.addSource("", "Some Name", null, null, null, null, null, null);
  ...
  void addSourceWithLlmsTxtUrlSavesIt() {
    ...
    mcpToolService.addSource(
        ...
        null,
        null,
        "https://docs.spring.io/llms.txt",
        null);

    ArgumentCaptor<Source> sourceCaptor = ArgumentCaptor.forClass(Source.class);
    verify(sourceRepository).save(sourceCaptor.capture());
    assertThat(sourceCaptor.getValue().getLlmsTxtUrl())
        .isEqualTo("https://docs.spring.io/llms.txt");
  }
  ...
  void removeSourceDeletesById() {
    ...
    Source source = new SourceBuilder().name("Spring Docs").status(SourceStatus.INDEXED).build();
  ...
  void recrawlSourceTriggersIncrementalByDefault() {
    ...
    verify(mcpToolService)
        .dispatchCrawl(eq(uuid), eq("https://docs.spring.io"), any(CrawlScope.class), eq(false));
  }
  ...
  void recrawlSourceWithFullFlagClearsStateFirst() {
    ...
    String output =
        mcpToolService.recrawlSource(uuid.toString(), true, null, null, null, null, null, null);

    assertThat(output).contains("full");
    verify(ingestionService).clearIngestionState(uuid);
    verify(mcpToolService)
        .dispatchCrawl(eq(uuid), eq("https://docs.spring.io"), any(CrawlScope.class), eq(true));
  }
  ...
  void recrawlSourceRejectsActivelyRunningCrawl() {
    UUID uuid = UUID.randomUUID();
    Source source = new SourceBuilder().name("Spring Docs").status(SourceStatus.CRAWLING).build();
    given(sourceRepository.findById(uuid)).willReturn(Optional.of(source));
  ...
  void recrawlSourceRejectsUpdatingSource() {
    UUID uuid = UUID.randomUUID();
    Source source = new SourceBuilder().name("Spring Docs").status(SourceStatus.UPDATING).build();
    given(sourceRepository.findById(uuid)).willReturn(Optional.of(source));
  ...
  void recrawlSourceWithInvalidIdReturnsError() {
    String output =
        mcpToolService.recrawlSource("not-a-uuid", null, null, null, null, null, null, null);
  ...
  void searchDocsPassesFilterParamsToSearchRequest() {
    given(searchService.search(searchRequestCaptor.capture()))
        .willReturn(
  ...
  void addSourceSetsVersionOnEntity() {
    ...
    mcpToolService.addSource(
        "https://docs.spring.io", "Spring Docs", null, null, null, null, null, "React 19");
  ...
  void listSourcesShowsRealChunkCountWithContentTypeBreakdown() {
    ...
    List<Object[]> grouped =
        List.<Object[]>of(new Object[] {"prose", 892L}, new Object[] {"code", 355L});
    given(documentChunkRepository.countBySourceIdGroupedByContentType(any())).willReturn(grouped);
  ...
  void indexStatisticsReturnsFormattedGlobalStats() {
    ...
    given(sourceRepository.findMaxLastCrawledAt())
        .willReturn(Instant.parse("2026-02-20T14:30:00Z"));
  ...
  void crawlStatusCompletedSummaryShowsRealChunkCountWithBreakdown() {
    ...
    given(progressTracker.getProgress(uuid)).willReturn(Optional.empty());
    List<Object[]> grouped =
        List.<Object[]>of(new Object[] {"prose", 50L}, new Object[] {"code", 20L});
    given(documentChunkRepository.countBySourceIdGroupedByContentType(any())).willReturn(grouped);
