 ...
 *
 * <ol>
 *   <li>llms.txt / llms-full.txt (machine-readable documentation index)
 *   <li>sitemap.xml (via {@link SitemapParser})
 *   <li>BFS link crawling (caller seeds with root URL)
 * </ol>
 */
@Service
public class PageDiscoveryService {
  ...
  /**
   * Discover URLs for a site using the priority cascade: llms.txt/llms-full.txt first, then
   * sitemap.xml, then link crawl fallback.
   *
  ...
  private @Nullable DiscoveryResult tryLlmsFullTxt(String baseUrl) {
    ...
    if (parsed.isFullContent()) {
      log.info(
          "Discovered llms-full.txt at {} ({} inline URLs)", llmsFullUrl, parsed.urls().size());
