 ...
 *
 * <p>Uses headless Chromium via Crawl4AI with a {@code PruningContentFilter} for boilerplate
 * removal. Prefers {@code fit_markdown} (filtered) over {@code raw_markdown}. Returns a {@link
 * CrawlResult} with success/failure status for graceful pipeline handling.
 *
 * @see CrawlResult
 * @see Crawl4AiConfig
 */
...
public class Crawl4AiClient {
  ...
  /**
   * Crawl a single URL via Crawl4AI sidecar. Uses PruningContentFilter for boilerplate removal and
   * headless Chromium for JS rendering.
   */
  public CrawlResult crawl(String url) {
    ...
    try {
      response =
          restClient.post().uri("/crawl").body(request).retrieve().body(Crawl4AiResponse.class);
    ...
    if (response == null || !response.success() || response.results().isEmpty()) {
      return new CrawlResult(
          url, null, List.of(), false, "Crawl4AI returned no results for " + url);
    }
    ...
    String markdown =
        page.markdown() != null
                && page.markdown().fit_markdown() != null
                ...
                Map.of(
                    ...
                    Map.of(
                        ...
                        Map.of(
                            ...
                            "params",
                            Map.of("threshold", 0.48, "min_word_threshold", 20)))))));
  }
}
