...
/**
 * Search orchestration layer implementing two-stage retrieval: over-fetch with metadata
 * filters, then cross-encoder reranking.
 *
 * <p>Pipeline: embed query -> build metadata filter from SearchRequest -> over-fetch 50
 * candidates from EmbeddingStore (hybrid vector + keyword) -> delegate to RerankerService
 * for cross-encoder scoring -> return top maxResults.</p>
 */
@Service
public class SearchService {
    ...
    /**
     * Performs two-stage hybrid search: over-fetch candidates with metadata filters,
    ...
    public List<SearchResult> search(SearchRequest request) {
        if (request.rrfK() != null) {
            log.debug("rrfK={} provided on SearchRequest but cannot be applied per-request "
                    + "(store-level config in LangChain4j); using configured store value", request.rrfK());
        }
        ...
        return rerankerService.rerank(request.query(), candidates, request.maxResults(), request.minScore());
    }

    /**
     * Builds a composable LangChain4j Filter from SearchRequest filter parameters.
