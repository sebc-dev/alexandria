 ...
 *
 * <p>Each record stores a content hash of the last ingested version of a page, enabling the
 * ingestion pipeline to skip unchanged pages on re-crawl. The unique constraint on {@code
 * (source_id, page_url)} ensures one state record per page per source.
 *
...
@Table(
    ...
    uniqueConstraints = @UniqueConstraint(columnNames = {"source_id", "page_url"}))
