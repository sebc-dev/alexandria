...
- [ ] **Task 2.1**: Définir les entités Document et Chunk
  ...
  - Notes: MAX_CHUNK_CONTENT=8000, MAX_CHUNKS_PER_DOC=500
...
- [ ] **Task 2.2**: Définir les value objects de recherche
  ...
  - Notes: threshold=0.5, limit=10 par défaut
...
- [ ] **Task 4.1**: Définir le schema Drizzle (F4, F5, F19)
  ...
  - Action: UNIQUE sur source, HNSW avec m=16, ef_construction=64
  - Notes: Ajouter SET hnsw.ef_search=40 dans connexion

- [ ] **Task 4.2**: Configurer la connexion DB (F18)
  - File: `src/infrastructure/db/index.ts`
  - Action: READ COMMITTED isolation, SET ef_search=40 à la connexion
  - Notes: Graceful shutdown avec timeout

- [ ] **Task 4.3**: Créer la migration initiale
  - File: `drizzle/0000_initial.sql`
  - Action: CREATE EXTENSION vector, HNSW params explicites, ALTER DATABASE ef_search (F19)
  - Notes: Vérifier que ef_search est bien configuré
...
- [ ] **Task 6.3**: Implémenter le tool d'ingestion (F3, F17, F24)
  ...
  - Action: Validation taille (F24), warning chunks > 4000 chars
...
- [ ] **AC-1**: Given un fichier Markdown valide et un fichier de chunks JSON conforme au schema (< 8000 chars par chunk), when j'exécute `alexandria ingest doc.md -c chunks.json`, then le document est stocké avec ses chunks vectorisés et je reçois le document ID.
...
- [ ] **AC-4**: Given un fichier de chunks avec content > 8000 chars (F24), when j'exécute la CLI, then j'obtiens erreur CONTENT_TOO_LARGE (-32008).
...
- [ ] **AC-5**: Given des documents ingérés, when je recherche avec threshold=0.5, then je reçois uniquement les chunks avec score >= 0.5, incluant chunk_index, heading, total_chunks (F29).
...
- [ ] **AC-8**: Given une query vide ou whitespace-only, when je recherche, then j'obtiens erreur VALIDATION_ERROR (-32002).
...
- [ ] **AC-11**: Given une erreur DB pendant la recherche, when le tool est appelé, then erreur -32004 (DATABASE_ERROR), serveur ne crash pas.

- [ ] **AC-11b**: Given erreur d'embedding pendant ingestion, when tool appelé, then erreur -32003 et rollback complet.

- [ ] **AC-11c**: Given tool `health` appelé pendant model loading (F23), then retourne immédiatement `{ model: "loading" }` sans bloquer.

- [ ] **AC-11d**: Given embedding retourne vecteur != 384 dimensions (F20), when ingestion, then erreur -32007 EMBEDDING_DIMENSION_MISMATCH.
...
- Database: PostgreSQL 18 local, données warm, ef_search=40 (F19)
- Model: Déjà chargé en mémoire

- [ ] **AC-12**: Given 1000 chunks indexés et cache warm, when je recherche, then temps < 500ms (p95).

- [ ] **AC-13**: Given un document de 50 chunks (< 8000 chars chacun) et modèle chargé, when je l'ingère, then temps total < 10s.

### Suppression

- [ ] **AC-14**: Given document existant, when `delete` par source, then document et chunks supprimés.

- [ ] **AC-15**: Given source inexistante, when `delete`, then erreur DOCUMENT_NOT_FOUND (-32001).
